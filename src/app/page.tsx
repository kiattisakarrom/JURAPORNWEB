...
import type { AlertKind, PatientQueueItem, Priority, QueueStage } from "@/types/pharmacy";
...
function PharmacyDashboard({ onLogout }: { onLogout: () => void }) {
  ...
  return (
    <main className="flex h-screen flex-col overflow-hidden bg-slate-100 text-slate-900">
      ...
      <div
        className={cn(
          "grid min-h-0 flex-1 overflow-hidden bg-white",
          selectedPatient && "lg:grid-cols-[minmax(0,1fr)_430px]",
        )}
      >
  ...
  return (
    <aside className="min-h-0 overflow-y-auto border-t border-slate-200 bg-white lg:border-l lg:border-t-0">
      ...
      <div className="space-y-6 p-5">
        ...
        <Button className="h-14 w-full border-dashed border-blue-200 text-blue-700" variant="outline">
          <Search className="h-5 w-5" />
          ตรวจสอบเครื่องและสต็อก
        </Button>
