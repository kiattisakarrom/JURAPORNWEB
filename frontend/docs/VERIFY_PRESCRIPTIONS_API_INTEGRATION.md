...
Backend แบ่งหน้าตามคีย์ `PATIENTID + VISITDATETIME + VISITNUMBER` และคืน PN กับ ITEMS ของ VN ที่เลือกมาครบชุด:

- `PAGINATION.TOTAL_PATIENTS` นับจำนวนผู้ป่วยที่ไม่ซ้ำ
- `PAGINATION.TOTAL_VISITS` นับจำนวน VN
- `PAGINATION.TOTAL_PAGES` คำนวณจากจำนวน VN

Frontend โหลดข้อมูลแบบสองช่วง:

1. เรียกหน้าแรกด้วย `limit=50` แล้วแสดง 50 VN แรกทันที
2. โหลดหน้าที่ 2 เป็นต้นไปเบื้องหลัง ชุดละไม่เกิน 4 request พร้อมกัน
3. ระหว่างโหลดจะแสดงสถานะ `กำลังโหลดข้อมูลเบื้องหลัง…` ที่แถบ Pagination
4. เมื่อครบทุกหน้าจึงแสดงจำนวน VN ที่แน่นอนและสถานะ `โหลดข้อมูลเบื้องหลังเสร็จแล้ว`
5. หากโหลดเบื้องหลังผิดพลาด หน้าแรกยังใช้งานได้และกดลองใหม่จากแถบด้านล่างได้
...
| `page` | ใช้งานใน API client | แบ่งหน้าตาม VN; หน้าแรกแสดงทันทีและหน้าที่เหลือโหลดเบื้องหลัง |
| `limit` | ใช้งานใน API client | ใช้ค่า 50 VN ต่อ Backend request |
